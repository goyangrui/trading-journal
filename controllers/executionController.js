...
const createExecution = async (req, res) => {
  ...
  // get the execution information and trade id that the user sent in the request body
  const { execution: executionInfo, tradeId } = req.body;

  // destructure the executionInfo object
  let {
    action: actionReq,
    execDate: execDateReq,
    positionSize: positionSizeReq,
    price: priceReq,
    commissions: commissionsReq,
    fees: feesReq,
  } = executionInfo;

  // if any of the execution information is missing
  if (
    actionReq === "" ||
    execDateReq === "" ||
    positionSizeReq === "" ||
    ...
    throw new BadRequestError("Missing fields in add execution row");
  }

  // otherwise
  // try and parse position size, price, commissions, and fees as floats
  try {
    positionSizeReq = parseFloat(positionSizeReq);
    priceReq = parseFloat(priceReq);
    commissionsReq = parseFloat(commissionsReq);
    feesReq = parseFloat(feesReq);
  } catch (error) {
    throw new Error();
  }

  // create new execution using the information provided by the user
  ...
  // get all of the executions with the given trade id
  const executionDocs = await Execution.find({ createdBy: userId, tradeId });
